...
class AABB:
  ...
  def __init__(self,low,high):
    ...
    self.centre = (0.5 * (self.x[0] + self.x[1] ),
                   0.5 * (self.y[0] + self.y[1]),
                   0.5 * (self.z[0] + self.z[1]))
...
class SetAABB:
  """A set of AABB's - uses a kd tree with hieuristic dimension detection
     to subdivide at each point. Each level keeps a list of aabb's that
     cross the dividing line."""
  def __init__(self,aabbs):
    """Given a list of AABB's."""
    # Work out what happens for dividing on each dimension - sort by the AABB's
    # centres and then select the centre aabb by volume, then try dividing by
    #the sides & centre of the centre aabb and count how many nodes are intercepted
    ...
    for dim in xrange(3):
      ...
      options = (byDim[centre].bounds[dim][0]-aabbLambda, byDim[centre].centre[dim],
                    byDim[centre].bounds[dim][1]+aabbLambda)
    ...
    # We have our bests - we now make this actual object,
    #                    and then recurse to make the full tree...
    zeroCount = 0
...
class Portal:
  ...
  def fromFace(self,aabb,dim,side):
    """Setup the portal from a face of the given aabb - you specify the dim
       of the face, with side==False meaning the low side and side==True meaning
       the high side. Will be anti-clockwise looking at it from inside the cube."""
    if side:
      side = 1
    else:
      side = 0
    ...
    ind = sum( map(lambda i:offsetC[1][i] * (offsetC[0][(i+1)%3] *
                       offsetC[2][(i+2)%3] - offsetC[0][(i+2)%3] *
                       offsetC[2][(i+1)%3]), xrange(3)))
  ...
  def setupPortal(self,portal,portalNode,flip):
    ...
    for o in order:
      portal.addVertex(Point3(self.verts[o][0] - c[0], self.verts[o][1] - c[1],
                                self.verts[o][2] - c[2]))



def findPortals(aabbs,overlap = 1e-3):
  """Given a list of AABB's this finds all intersections and creates portals.
     To store the portals creates a variable in each aabb,
     portals = [[[],[]],[[],[]],[[],[]]] - first index is dimension,
     second index is low (0) and high (1), final list is all portals using that face.
     Returns the portals as a list. Will throw an error if the geometry is bad.
     Will modify the dimensions of the given aabb's to account for overlap."""
  ...
  for dim in xrange(3):
    ...
    # Iterate all aabbs and create a push event and pop event for each - push it
    # on when you reach the minimum, pop it when you get to the maximum.
    # (True,d,aabb) to push, (False,d,aabb) to pop...
    events = []
    for aabb in aabbs:
    ...
    # Iterate through the events in sequence - each time a aabb is pushed or
    #popped check if it intercepts a face larger than it - if so add the relevant portal...
    #(Partial interception is considered an error as it results in ambiguous behaviour.
    #Multiple interception is also not allowed as its an entire face that intercepts
    #from our point of view. (Larger face can have multiple intercepts of course.))
    ...
    for event in events:
      ...
      for key,aabb in state.iteritems():
        ...
        if sum(withinCount)==4:
          ...
          # We have an interception - update the relevant aabb to have only the
          # slightest overlap then create the portal and finally arrange for all the links...
          if event[0]:
